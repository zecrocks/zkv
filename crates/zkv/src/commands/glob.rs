//! Redis `KEYS`-style glob matching, shared by the `keys`, `get`, and
//! `shallow` commands.
...
/// Whether `pattern` carries a glob wildcard (an unescaped `*`), i.e. it is a
/// pattern rather than a literal key. A `\*` is an escaped literal asterisk and
/// does **not** count. Callers use this to decide whether a key argument can be
/// resolved exactly (early-stop find) or must be matched across a window.
...
#[cfg(test)]
mod tests {
    use super::{glob_match, has_wildcard};
    ...
    fn wildcard_detection() {
        assert!(has_wildcard("prices/*"));
        assert!(has_wildcard("*"));
        assert!(has_wildcard("a*b"));
        assert!(!has_wildcard("prices/zec_usd"));
        assert!(!has_wildcard(""));
        // An escaped star is a literal, not a wildcard.
        assert!(!has_wildcard(r"foo\*"));
        assert!(!has_wildcard(r"\*"));
        // A real star after an escaped one still counts.
        assert!(has_wildcard(r"\**"));
    }
}
